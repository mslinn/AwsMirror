  ...
  def auth(action: AuthAction, accountName: String=null): Unit = {
    ...
    action match {
      case `add` =>
        ...
        val credentials = AllCredentials(parse[Array[Credentials]](fileContents))
      ...
      case `delete` =>
        ...
        val oldCredentials = AllCredentials(parse[Array[Credentials]](fileContents))
      ...
      case `list` =>
        if (!credentialPath.exists) {
          ...
          parse[Array[Credentials]](fileContents) foreach { creds: Credentials =>
      ...
      case `modify` =>
        ...
        if (credentialPath.exists) {
          val oldCredentials = parse[Array[Credentials]](fileContents)
