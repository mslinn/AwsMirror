...
trait Application {
  ...
  protected def register(xs: ArgInfo*) {
    xs foreach (argInfos += _)
  }
  ...
  private def isRealMain(m: Method) = cond(m.getParameterTypes) {
    case Array(CArrayString) => true
  }
  ...
  def getNumber(clazz: Class[_], value: String): Option[AnyRef] =
    try {
      (valueOfMap get clazz) map (_.invoke(null, value))
    }
    catch {
      ...
      case clazz: Class[_] =>
        if (valueOfMap contains clazz)
          ...
          getConv(clazz, value) getOrElse {
            try {
              clazz.getConstructor(CString).newInstance(value).asInstanceOf[AnyRef]
            }
            catch {
              case x: NoSuchMethodException => fail
            }
          }
